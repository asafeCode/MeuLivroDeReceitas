...
public class RecipeValidator : AbstractValidator<RequestRecipeJson>
{
    public RecipeValidator()
    {
        ...
        RuleForEach(recipe => recipe.Instructions).ChildRules(instRule =>
        {
            instRule.RuleFor(inst => inst.Step).GreaterThan(0)
                .WithMessage(ResourceMessagesException.NON_NEGATIVE_INSTRUCTION_STEP);

            instRule.RuleFor(inst => inst.Text).NotEmpty()
        ...
        RuleFor(recipe => recipe.Instructions)
            .Must(instructions =>
                instructions.Select(i => i.Step)
                    .Distinct().Count() == instructions.Count)
